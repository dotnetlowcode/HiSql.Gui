...
import { GraphEventArg } from '../interface/graph-core';
...
export default class EventController {
  ...
  public registerEvent = (events: GraphEventArg[]) => {
    events &&
      events.forEach((event: GraphEventArg) => {
        switch (event.eventName) {
          ...
          case 'selection:changed': {
            this.graph.on('selection:changed', ({ selected, removed, added }) => {
              event.handler && event.handler({ selected, removed, added });
              (this.x6BaseGraph as any).bringCellsToFront(selected);
            });
            break;
          }
          default: {

...
export default class X6BaseGraph implements BaseGraph {
  ...
  /**
   * 注册监听事件
   * @param {GraphEventArg[]} events
   * @memberof X6BaseGraph
   */
  public registerEvent(events: GraphEventArg[]) {
  ...
  private graphContentDiff(graphData: GraphData) {
    ...
    this.cellController.nodes.forEach((node: Node) => {
      const findNodeData = nodesData.find((nodeData: NodeConfig) => nodeData.id === node.id);
      if (!findNodeData) {
        removeNodes.push(node);
      }
      // !!! 目前仅支持节点data数据变化, 才认为更新
      else if (_.isEqual(node.data, findNodeData.data)) {
    ...
    this.cellController.edges.forEach((edge: Edge) => {
      const findEdgeData = edgesData.find((edgeData: EdgeConfig) => edgeData.id === edge.id);
      if (!findEdgeData) {
        removeEdges.push(edge);
      }
      // !!! 目前仅支持连线data数据变化, 才认为更新
      else if (_.isEqual(edge.data, findEdgeData.data)) {
